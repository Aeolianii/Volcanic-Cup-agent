import type { StorySeed } from "@/types";

export interface StoryAnalysis {
  playable: boolean;
  playable_score: number; // 0-100
  features: StoryFeaturesAnalysis;
  missing_elements: string[];
  suggested_fixes: string[];
}

export interface StoryFeaturesAnalysis {
  has_conflict: boolean;
  has_mystery: boolean;
  has_factions: boolean;
  has_relationships: boolean;
  has_survival_pressure: boolean;
  has_resource_competition: boolean;
}

export function analyzeStory(seed: StorySeed): StoryAnalysis {
  const features = extractSeedFeatures(seed);
  const missing: string[] = [];
  const fixes: string[] = [];

  let score = 50;

  if (!features.has_conflict) {
    missing.push("核心冲突");
    fixes.push("建议加入主要矛盾或对立势力");
    score -= 15;
  } else {
    score += 10;
  }

  if (!features.has_mystery) {
    missing.push("悬疑/谜团");
    fixes.push("建议加入未解之谜或隐藏真相");
    score -= 5;
  } else {
    score += 10;
  }

  if (!features.has_factions) {
    missing.push("阵营/势力");
    fixes.push("建议加入至少两个利益冲突的阵营");
    score -= 10;
  } else {
    score += 10;
  }

  if (!features.has_relationships) {
    missing.push("角色关系网");
    fixes.push("建议为角色之间添加关系设定");
    score -= 5;
  } else {
    score += 5;
  }

  // Check for enough characters
  const charMentions = seed.characters.split(/[,，、\s]+/).filter(Boolean);
  if (charMentions.length < 3) {
    missing.push("角色数量不足（至少需要 3 个角色）");
    fixes.push("建议添加更多角色以支持多人互动");
    score -= 10;
  } else {
    score += 5;
  }

  // Check setting completeness
  if (!seed.world_setting || seed.world_setting.length < 10) {
    missing.push("世界观设定不够详细");
    fixes.push("建议详细描述世界观背景");
    score -= 5;
  }

  return {
    playable: score >= 40,
    playable_score: Math.max(0, Math.min(100, score)),
    features,
    missing_elements: missing,
    suggested_fixes: fixes,
  };
}

function extractSeedFeatures(seed: StorySeed): StoryFeaturesAnalysis {
  const text = `${seed.genre} ${seed.opening} ${seed.ending} ${seed.characters} ${seed.world_setting}`.toLowerCase();

  return {
    has_conflict: /冲突|战争|对抗|敌对|争夺|权谋|阴谋|背叛/.test(text),
    has_mystery: /谜|秘密|真相|悬疑|调查|推理|失踪|失窃/.test(text),
    has_factions: /阵营|势力|国家|家族|组织|教派|王国|帝国/.test(text),
    has_relationships: /关系|爱情|友情|亲情|婚姻|联盟|师徒/.test(text),
    has_survival_pressure: /生存|末日|饥荒|瘟疫|灾难|危险|死亡/.test(text),
    has_resource_competition: /资源|财富|宝藏|圣杯|神器|魔法|领地/.test(text),
  };
}
