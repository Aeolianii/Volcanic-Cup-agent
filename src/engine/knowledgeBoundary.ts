...
export function buildVisibleFactionsForPlayer(
  ...
  if (!usesFactionSecrecy(bible)) return [];
  ...
  return bible.factions.reduce<VisibleFaction[]>((visible, faction) => {
    ...
    for (const member of faction.members) {
      if (
        ...
        knownFacts.has(`public_claim:${member}:${faction.id}`) ||
...
export function buildChatChannelsForPlayer(
  ...
  const noFactionMode = !usesFactionSecrecy(bible);
  ...
  if (factionId && usesFactionSecrecy(bible)) {
  ...
  } else if (bible.factions.length > 0) {
    channels.push({
      id: "faction:judgement",
      type: "system",
      label: "阵营判断",
      member_ids: [player.player_id],
      pinned: true,
      unread_count: 0,
      unlocked: true,
    });
  ...
  for (const other of roomPlayers) {
    ...
    channels.push({
      ...
      type: "private",
      label: `私聊：${other.role?.name || other.name}${other.is_ai ? "（AI代演）" : ""}`,
...
function usesFactionSecrecy(bible: StoryBible): boolean {
  const profile = inferScriptProfile(bible);
  if (profile === "romance" || profile === "relationship" || profile === "mystery") return false;
  return Boolean(bible.runtime_modules?.enabled.factions || bible.factions.length > 0);
}
