...
export function validateStoryBible(bible: StoryBible): ValidationResult {
  ...
  for (const npc of bible.npcs) {
    if (!npc.goal) {
      errors.push({ field: `npcs[${npc.name}].goal`, message: `NPC ${npc.name} 必须有 goal` });
    }
  ...
  for (const event of bible.events) {
    if (!event.trigger) {
      errors.push({ field: `events[${event.title}].trigger`, message: `事件 ${event.title} 必须有 trigger` });
    }
