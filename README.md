...
AI Story Engine 是一个 AI 驱动的多人互动叙事引擎。用户输入故事创意后，系统生成 Story Bible，并围绕 Story Bible 构建动态规则、动态 UI、多人房间、AI GM、AI NPC、World State 和多结局流程。
...
- 新增 `public/images/home-background.png` 首页背景图资源，并将其接入首页导航线下方的完整背景区域。
- 首页背景不再受内容容器宽度限制，使用整屏宽度展示，避免只覆盖按钮所在区域。
- 背景图改为按宽度等比例显示，保留图片左右两侧场景；下方通过暗色渐变与页面背景自然衔接。
- 保持首页标题、入口按钮、加入房间表单和功能模块结构不变，仅调整背景层、遮罩和可读性。

#### AI 故事生成与剧本完善

- 创建故事页新增“随机故事”能力，可由 LLM 自动生成题材、开场、结局方向、人物、人物详情和世界观，并自动回填 6 个输入框。
- 新增 `/api/stories/random` 随机故事接口，支持 LLM 生成与本地随机故事兜底。
- 增强 LLM JSON 容错解析：当模型返回 JSON 不完整时，系统会尝试自动提取可用字段，避免直接暴露长段 SyntaxError。
- 低可玩性分析新增“由 AI 完善剧本和人设”流程，调用 `/api/stories/enhance` 补强人物性格、公开目标、秘密目标、关系冲突、线索和世界观约束。

#### 角色生成与目标系统修复

- 修复第 5 框“人物性格和人物间关系”被合并进 `characters`，导致公开目标、秘密目标等内容被当作可选人物的问题。
- Story Bible 新增 `character_details` 字段，角色名单与人物详情分离处理。
- 角色身份生成会读取人物详情，优先生成具体身份，不再统一显示“卷入核心冲突的重要人物”。
- 修复公开目标解析时混入秘密目标的问题，目标字段会在“秘密目标 / 秘密目的 / 隐藏动机 / 真实目标”等标签处自动截断。
- 增强秘密目标生成：当 AI 未提供足够具体的秘密目标时，会根据角色身份生成更贴合的隐藏动机。

#### AI GM 与推荐行动体验修复

- AI GM 调用失败时不再直接中断回合体验，也不使用模板剧情冒充真实生成。
- 新增基于真实规则结算数据的“回合结算摘要型叙事”，展示玩家行动结果、公开指标变化、触发事件和 NPC 动向。
- 推荐行动面板新增内部 ID 清洗，避免 `current_location`、`self_goal`、`npc_xxx`、`event_xxx`、`location_xxx` 等内部字段泄露到玩家界面。
- 推荐行动标准化阶段会同步清洗 `label`、`context` 和 `target`，显示层也增加兜底清洗。

#### 文档与美术设计资产

- 新增 P0 优先级美术设计提示词文档：`P0_图片生成提示词_美术设计.docx`。
- 文档覆盖首页 Hero、创建故事页 Banner、Story Bible 封面、AI GM 形象、结局页主视觉、游戏房间背景、角色卡和低可玩性提示插图等关键素材。
- README 版本记录更新至 v1.8，说明当前 UI、美术、LLM、角色目标和推荐行动修复内容。

#### 验证

- 已通过 `npm run build`。
- 已本地重启并验证 `http://localhost:3000/` 首页可访问。
- 已验证 `http://localhost:3000/images/home-background.png` 资源可返回 200。
